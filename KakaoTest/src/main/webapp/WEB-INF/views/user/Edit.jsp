		...
		<div class="sub-conts join util-width find-finish" id="mainUserUpdate">
			...
			<div class="cardList-wrap">
				...
				<div class="btn-group">
					...
					<c:if test="${not empty user.kakaoId}">
						<form action="kakao/delete" method="post">
							<input type="hidden" name="kakaoId" value="${user.kakaoId}">
							<input type="hidden" name="accessToken" value="${user.accessToken}">
							<button class="btns1 md-ripples ripples-light"
							type="submit" style="background-color: gold">카카오계정 연동 해제</button>
						</form>
					</c:if>
				</div>
			</div>
